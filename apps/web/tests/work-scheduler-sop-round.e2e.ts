...
const PROMPT =
  'Use sync_work_scheduler exactly once with workflow "Development SOP" and stages intake completed, implement in_progress, and verify pending. Use names Intake, Implementation, and Verification. Then reply exactly SOP_SYNC_DONE and stop.'
...
describe('web e2e: SOP stages synchronize into the Workspace board', () => {
  ...
  it('executes the scoped tool and persists a Session-bound projection', async () => {
    ...
    const workspace = scaffold.ctx.workspaceRegistry.list().find(item => item.sessionIds.includes(sessionId))
  ...
  it('opens windowed, changes display modes, and follows the bound Session', async () => {
    ...
    await page
      .getByRole('button', { name: /^(?:New session|新.*会话)$/ })
      .last()
      .click()
    ...
    await dialog.getByRole('textbox', { name: '任务描述' }).fill('校验新建任务体验')
    await dialog.getByRole('textbox', { name: '验收条件（每行一项）' }).fill('任务可以保存\n返回看板保留筛选')
    expect(await dialog.locator('main').isVisible()).toBe(false)
    ...
    expect(await dialog.getByLabel('Git 起始版本').isVisible()).toBe(false)
    const detailSnapshot = await captureStableAria(
      page,
      '[role="dialog"][aria-label="工作调度"]',
      scaffold.workspaceCwd,
    )
    await compareOrRefreshGolden(
      fileURLToPath(new URL('./snapshots/work-scheduler-sop-round/details.expected.md', import.meta.url)),
      detailSnapshot,
      MODE,
    )
    ...
    await dialog.getByRole('button', { name: /Implementation/ }).click()
    await dialog.getByRole('button', { name: '打开关联会话' }).click()
