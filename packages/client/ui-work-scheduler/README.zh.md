...
侧栏入口和全屏浮层共享同一个插件自有 store。面板从客户端 runtime 读取 Session 与 Workspace 注册表，并通过 connection 插件进行持久化加载和保存。移除客户端条目会同时撤销两个 slot 贡献。可安装的 [`dsh-work-scheduler`](../../bundle/work-scheduler/README.zh.md) bundle 在 Web 表层所有者之后挂载本插件及其 Host 存储。
