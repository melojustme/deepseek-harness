...
| `@deepseek-ai/dsh-tool-work-scheduler` | `sync_work_scheduler` | `ctx.tools`, `ctx.workspaceRegistry`, `ctx.workSchedulerStore`, `a calling root Agent` | `tool/call`, `the calling Session's SOP projection in its Workspace scheduler document`, `tool/result` | - | Registered in each root Agent scope by the work-scheduler bundle. The tool resolves the calling Session's Workspace and replaces only that Session's deterministic SOP task namespace. |
...
Synchronize the complete ordered development-SOP stage list into the current Session's Workspace work scheduler. Call after every SOP phase transition. The tool resolves Workspace and Session ids from the calling agent, preserves manual work, and replaces only this Session's SOP tasks. A Session that is not assigned to a Workspace fails without writing.
...
Source: [`packages/work-scheduler/tool-work-scheduler/src/tool.ts`](../packages/work-scheduler/tool-work-scheduler/src/tool.ts)

Registered in each root Agent scope by the work-scheduler bundle. The tool resolves the calling Session's Workspace and replaces only that Session's deterministic SOP task namespace.
