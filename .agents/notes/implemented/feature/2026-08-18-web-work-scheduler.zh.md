...
`@deepseek-ai/dsh-client-ui-work-scheduler` 向 `sidebar.footer.action` 贡献入口，向 `shell.overlay` 贡献模态层，两个条目通过同一个插件 store 共享打开状态和调度文档。打开任务所关联的 Session 时，插件通过客户端 runtime 选中该 Session 并关闭完整模态层，以显示对应对话。该方案使用已声明的 UI 扩展点，不改变会话所有者或 agent loop。
...
任务创建在命令区内按需展开。收起状态会把次要的新建线程操作与主要的新建任务操作分开。打开任务编辑器后，多行描述会自动获得焦点；待分配区和线程以可横向滚动的单选组呈现；Session 选择器仅搜索当前 Workspace。提交、取消和关闭模态层都会丢弃草稿；按 `Escape` 会依次关闭 Session 选择器、编辑器和模态层。展开的编辑器不会成为移动端页面的滚动所有者，因此看板仍可独立访问。
...
**保留由原生下拉框组成的单行任务表单。** 单行布局会让描述、位置、关联和提交操作争夺宽度，较长的 Session 标题也难以浏览。按需展开会在用户明确新建任务前保持看板紧凑，并为位置和 Session 选择提供足够的可读空间。

## 后果

随附 Web profile 依次组合 base、Web app 与调度 bundle。它从侧栏提供调度入口，在 Host 重启后保留调度文档，并且在不增加全局工具的情况下向 root Agent 提供 SOP 同步工具。纯状态转换测试固定阻塞、唤醒、归档、拖放位置、导入规范化、异步返回位置、按 Session 隔离的 SOP 替换、幂等性和阶段回退。浏览器组件与随附组合测试固定窗口位置和大小、全屏还原、响应式全屏、任务编辑器重置与分层关闭、可搜索 Session 选择、移动端滚动所有权、Session 导航、SQLite 持久化及 Agent 作用域注册。并发编辑采用后写胜出，删除 Workspace 后仍会留下调度文档，预发布版本 2 格式没有迁移路径。调度状态可以定位 Session，但对 Session、工作流、后台作业和子 agent 没有控制权。
